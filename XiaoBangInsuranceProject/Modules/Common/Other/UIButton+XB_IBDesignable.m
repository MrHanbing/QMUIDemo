...
#import "UIButton+XB_IBDesignable.h"
#import "XBDeVice.h"
@implementation UIButton (XB_IBDesignable)


static char kAssociatedObjectKey_consultFontSize;
-(void)setConsultFontSize:(CGFloat)consultFontSize{
    if (consultFontSize<= 0 ) {
        return;
    }
    CGFloat fontsize = consultFontSize *   [XBDeVice pixelPerInchScal];
    self.titleLabel.font = [UIFont systemFontOfSize:fontsize < self.miniFontSize?self.miniFontSize:fontsize];
